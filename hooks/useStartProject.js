...
import { tryCatchWrapper } from '../utils/tryCatch.util';
...
export const useStartProject = (name, packageManager, setLoadingMsg, updateSteps, setCompleted, exit) => {
	...
	const installDependencies = async (dest, info) => {
		...
		await tryCatchWrapper(createLibraryModule.initPackagesRoot, { dest, info }, updateSteps, STEPS.INSTALL_ROOT)
		setLoadingMsg('Installing dependencies in workbench')
		await tryCatchWrapper(createLibraryModule.initPackagesWorkbench, { dest, info }, updateSteps, STEPS.INSTALL_ROOT)
	}
